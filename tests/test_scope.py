...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            _cluster(us_cite=None, us_volume=None, us_page=None),
            scope.IsScotus.FALSE,
            "no_us_reports_cite",
        ),
        # out-of-scope volumes: pre-SCOTUS Dallas vol 1, and the 1822-term vol-20 straggler
        (_cluster(us_volume=1, us_cite="1 U.S. 1"), scope.IsScotus.FALSE, "out_of_scope_volume"),
        (_cluster(us_volume=20, us_cite="20 U.S. 1"), scope.IsScotus.FALSE, "out_of_scope_volume"),
...
def test_not_scotus_tells(name, volume, page, expected):
    tells = scope.not_scotus_tells({"case_name": name, "us_volume": volume, "us_page": page})
    if expected:
...
def test_page_before_scotus_start_tell():
    tells = scope.not_scotus_tells({"case_name": "Some PA Case", "us_volume": 2, "us_page": "384"})
...
def _scope_real_staging_readonly():
...
def test_no_not_scotus_case_is_kept():
    ...
    kept = [
        cid
        for cid, disposition in _answer_key().items()
